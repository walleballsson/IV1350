...
public class RepairOrderRegistryTest {
    ...
    public void testCreatedOrderHasCorrectId() {
        RepairOrder order = registry.createRepairOrder(customer, bike, "Flat tire");
        assertEquals(1, order.getId(), "First order should have id 1");
    }

    @Test
    public void testSecondOrderHasIncrementedId() {
        registry.createRepairOrder(customer, bike, "Flat tire");
        RepairOrder second = registry.createRepairOrder(customer, bike, "Battery dead");
        assertEquals(2, second.getId(), "Second order should have id 2");
    }

    @Test
    public void testFindByIdReturnsCorrectOrder() {
        RepairOrder created = registry.createRepairOrder(customer, bike, "Flat tire");
        RepairOrder found = registry.findById(created.getId());
        assertEquals(created.getId(), found.getId(), "Found order should have same id");
    }
